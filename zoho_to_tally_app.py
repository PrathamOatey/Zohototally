...
def process_chart_of_accounts(df):
    ...
    } # Closing curly brace was missing or misplaced in previous versions
    ...
    if 'Opening Balance' in df_mapped.columns:
        ...
        # Default to 0.00 if column not found in data
...
def process_contacts(df):
    ...
    for col in numeric_cols: # Corrected missing colon
        if col in df_cleaned.columns: # Check for column existence
...
def process_invoices(df):
    ...
    numeric_cols = [
        ...
        'Reverse Charge Tax Name', 'Reverse Charge Tax Rate', 'Reverse Charge Tax Type', # Added missing GST columns
        'Item TDS Percentage', 'Item TDS Amount', # Corrected names
    ...
    string_cols = [
        ...
        'Supplier ZipCode', 'Supplier Phone', 'Supplier E-Mail', # Removed duplicate GST rates/amounts which are numeric_cols
...
def generate_contacts_vendors_xml(df_contacts, df_vendors):
    ...
    if df_contacts is not None:
        for index, row in df_contacts.iterrows():
            party_name = safe_str(row.get('Tally_Party_Name', '')) # Use .get() for party_name here too
            if not party_name:
...
def generate_sales_vouchers_xml(df_invoices):
    ...
    for invoice_id, group in grouped_invoices:
        ...
        if round_off_amount != 0.0: # math.isnan implies it's already a float or 0.0
